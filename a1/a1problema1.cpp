...
int main() {
    ...
    int opt;
    double temp;
    cout << "Welcome!" << endl << "1. Farenheit to Celsius" << endl;
    cout << "2. Celsius to Farenheit" << endl;
    cin >> opt;
    cout << "Selection: " << opt << endl;
    cout << "Introduce la temperatura en " << ((opt!=1)? "Celsius" : "Farenheit") << ": ";
    cin >> temp;
    cout << "La temperatura en " << ((opt==1)? "Celsius" : "Farenheit") << " es " << ((opt!=1)? C2F(temp) : F2C(temp));
    cout << " " << char(248) << ((opt==1)? "C" : "F") << endl;
    cin.get();
    return 0;
}
